...
### 1. Core Logic
 ...
 -   **File Input & Compression**: The program reads an arbitrary input file (`-inputFile`). The entire content of this file is then compressed using **Gzip compression** (from Go's `compress/gzip` standard library).
 -   **Chunking**: The resulting gzipped data is then split into byte chunks based on `-chunkSize`. This `chunkSize` refers to the size of the *gzipped* data portions.
-   **Metadata Prepending (Chunk Header Structure)**: Before QR code generation, each *gzipped* data chunk is prepended with a header. The fields are serialized in the following order (all BigEndian):
    1.  `Checksum` (uint64, 8 bytes): XXH64 checksum.
    2.  `TotalChunks` (uint16, 2 bytes): The total number of *gzipped* chunks (max 65,535).
    3.  `SequenceNum` (uint16, 2 bytes): The 0-indexed sequence number of the current chunk (max 65,535).
    4.  `DataLength` (uint16, 2 bytes): The length of the `OriginalData` (gzipped data) portion of the current chunk in bytes (max 65,535).
...
-   **Checksum Calculation**: The `Checksum` is calculated over the byte representations of the following fields concatenated in order: `TotalChunks` (2 bytes) + `SequenceNum` (2 bytes) + `DataLength` (2 bytes) + `OriginalData` (gzipped data chunk, variable length).
-   **Final QR Payload**: The complete binary data that is encoded into the QR code consists of the full 14-byte header followed by the `OriginalData` (gzipped data chunk).
...
-   **Go Standard Library**: Used for file operations, command-line flags, `os/exec`, `encoding/binary`, `encoding/ascii85`, `compress/gzip`, `bytes`, `math`, etc.
...
-   **Go Modules**: The project is a single Go module at the repository root (e.g., `qrvidproject`). The root `go.mod` and `go.sum` files manage dependencies for both `encode` and `decode` packages. Run `go mod tidy` from the repository root if dependencies change. Note: `encoding/ascii85` and `compress/gzip` are part of the Go standard library.
...
-   **Chunk Payload Structure**: Be mindful of the new header structure: `[Checksum (8B)][TotalChunks (2B uint16)][SequenceNum (2B uint16)][DataLength (2B uint16)]` followed by `[OriginalData]`. The `OriginalData` here is a chunk of the **Gzipped** file content. The total header size is 14 bytes. Any changes to this must be reflected in the decoder(s). The `chunkSize` parameter refers only to the size of these (gzipped) `[OriginalData]` parts. The binary data (header + gzipped OriginalData chunk) is ASCII85 encoded before being turned into a QR code. Max values for TotalChunks, SequenceNum, and DataLength are 65,535.
-   **Flag Validation**: Input flags like `-resolution`, `-crf`, etc. have specific formats or allowed values. Ensure these are validated. The encoder will also error if `TotalChunks`, `SequenceNum` (chunk index), or `DataLength` (gzipped chunk data length) would exceed `math.MaxUint16`.
