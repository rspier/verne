...
### 1. Core Logic
 ...
 -   **File Input & Compression**: The program reads an arbitrary input file (`-inputFile`). The entire content of this file is then compressed using Zstandard (zstd) compression.
 -   **Chunking**: The resulting compressed data is then split into byte chunks based on `-chunkSize`. This `chunkSize` refers to the size of the *compressed* data portions.
-   **Metadata Prepending (Chunk Header Structure)**: Before QR code generation, each *compressed* data chunk is prepended with a header. The fields are serialized in the following order (all BigEndian):
    1.  `Checksum` (uint64, 8 bytes): XXH64 checksum.
    2.  `TotalChunks` (uint16, 2 bytes): The total number of *compressed* chunks (max 65,535).
...
-   **Chunk Payload Structure**: Be mindful of the new header structure: `[Checksum (8B)][TotalChunks (2B uint16)][SequenceNum (2B uint16)][DataLength (2B uint16)]` followed by `[OriginalData]`. The `OriginalData` here is a chunk of the ZSTD-compressed file content. The total header size is 14 bytes. Any changes to this must be reflected in the decoder(s). The `chunkSize` parameter refers only to the size of these (compressed) `[OriginalData]` parts. The binary data (header + compressed OriginalData chunk) is ASCII85 encoded before being turned into a QR code. Max values for TotalChunks, SequenceNum, and DataLength are 65,535.
-   **Flag Validation**: Input flags like `-resolution`, `-crf`, etc. have specific formats or allowed values. Ensure these are validated. The encoder will also error if `TotalChunks`, `SequenceNum` (chunk index), or `DataLength` (compressed chunk data length) would exceed `math.MaxUint16`.
