...
*   View messages within a newsgroup (`/group/{groupname}/{year}/{month}.html`):
    *   Messages are displayed hierarchically using JWZ threading.
    *   Smarter navigation links (only to months with content).
*   Show other messages in the same thread (flat list) at the bottom of an article page.
...
*   `github.com/gatherstars-com/jwz` for JWZ message threading.
*   `github.com/stretchr/testify/assert` for enhanced testing assertions.
*   Docker (for containerized deployment)
*   (Note on `safehtml`: Using `html/template` for parsing; `safehtml` types are not directly used. HTML sanitization is handled by `bluemonday`.)
...
### Command-line Flags
Configuration can be provided via command-line flags. Corresponding environment variables (prefixed with `NNTPWEB_`) can also be used, but command-line flags take precedence.

*   `-db-host`: Database host (env: `NNTPWEB_DB_HOST`, default: `localhost`)
*   `-db-port`: Database port (env: `NNTPWEB_DB_PORT`, default: `3306`)
*   `-db-user`: Database user (env: `NNTPWEB_DB_USER`, default: `user`)
*   `-db-pass`: Database password (env: `NNTPWEB_DB_PASS`, default: `password`)
*   `-db-name`: Database name (env: `NNTPWEB_DB_NAME`, default: `nntp_cache`)
*   `-server-port`: HTTP server port (env: `NNTPWEB_SERVER_PORT`, default: `8080`)
*   `-nntp-server`: Backend NNTP server address (host:port) (env: `NNTPWEB_NNTP_SERVER`, default: `news.example.com:119`) - Used by the internal NNTP client to fetch article bodies.
*   `-cache-ttl`: Default cache TTL in seconds for database query results (env: `NNTPWEB_CACHE_TTL`, default: `300`).
```
