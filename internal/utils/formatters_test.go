...
func TestObfuscateEmailInFromHeader(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "empty name, email present (mail.ParseAddress behavior check)",
			from:     "<> <user@example.com>",
			expected: "<> <user@example.com>",    // If ParseAddress gives Name="<>" -> returns "<>"; if ParseAddress fails -> returns original. Test log implies original.
		},
		{
