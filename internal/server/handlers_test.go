...
import (
	...
	"github.com/stretchr/testify/assert" // Added for assert.Contains, assert.Regexp etc.
)
...
func TestServer_handleListMessages_JWZThreading(t *testing.T) {
	...
	articleQuery := regexp.QuoteMeta(`
		...
		FROM articles
		WHERE group_id = ? AND YEAR(received) = ? AND MONTH(received) = ?
		ORDER BY received DESC, id DESC`) // Note: JWZ relies on Date header, not received, but query uses received.
	prevMonthNavQuery := regexp.QuoteMeta(`
		SELECT YEAR(received), MONTH(received)
		FROM articles
		WHERE group_id = ? AND received < ?
	...
	// Articles for threading:
	// msg1 (root)
	// msg2 (reply to msg1)
	// msg3 (reply to msg2)
	// msg4 (another reply to msg1)
	// msg5 (root, different thread)
	// Dates for h_date (used by JWZ) need to be parsable by mail.ParseDate
	// Dates for received (used by DB query) need to be in "YYYY-MM-DD HH:MM:SS"
	...
	time4 := time1.Add(2 * time.Hour) // after time2, but reply to time1
	time5 := time1.Add(30 * time.Minute)
	...
	mockArticles := []struct {
		...
		msgID       string
		subject     string
		hDate       string // For JWZ parsing
		received    time.Time // For DB query & sorting
		references  string // Raw references string
	}{
		{1, "msg1@host.jwz", "Root Message 1", time1.Format(time.RFC1123Z), time1, ""},
		{2, "msg2@host.jwz", "Re: Root Message 1", time2.Format(time.RFC1123Z), time2, "<msg1@host.jwz>"},
		{3, "msg3@host.jwz", "Re: Re: Root Message 1", time3.Format(time.RFC1123Z), time3, "<msg2@host.jwz>"},
		{4, "msg4@host.jwz", "Re: Root Message 1 (alt)", time4.Format(time.RFC1123Z), time4, "<msg1@host.jwz>"},
		{5, "msg5@host.jwz", "Root Message 2", time5.Format(time.RFC1123Z), time5, ""},
	}
	...
	setupMock := func(mock sqlmock.Sqlmock) {
		...
		for _, ma := range mockArticles {
			// group_id, id, h_messageid, h_subject, h_from, h_date, received, thread_id, parent, h_references, h_lines, h_bytes
			rows.AddRow(groupID, ma.id, ma.msgID, ma.subject, "Test User", ma.hDate, ma.received, ma.id, 0, ma.references, 10, 100) // simplified thread_id/parent for mock
		}
		mock.ExpectQuery(articleQuery).WithArgs(groupID, targetYear, targetMonth).WillReturnRows(rows)

		// Prev/Next month navigation (mock them to return no links for simplicity)
		mock.ExpectQuery(groupQuery).WithArgs(groupName).WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(groupID))
		prevNavTargetDate := time.Date(targetYear, time.Month(targetMonth), 1, 0,0,0,0,time.UTC)
		mock.ExpectQuery(prevMonthNavQuery).WithArgs(groupID, prevNavTargetDate).WillReturnError(sql.ErrNoRows)
	...
	// Check for correct nesting based on the template structure.
	// Example: Root Message 1 should appear, then Re: Root Message 1 indented, etc.
	// This relies on the template using something like <ul class="thread-children"> for indentation.
	// And <li class="thread-level-X">

	// Expected structure (simplified):
	// Root Message 1 (level 0)
	//   Re: Root Message 1 (level 1, child of msg1)
	//     Re: Re: Root Message 1 (level 2, child of msg2)
	//   Re: Root Message 1 (alt) (level 1, child of msg1)
	// Root Message 2 (level 0)

	// Check for presence of subjects
	assert.Contains(t, body, "Root Message 1")
	assert.Contains(t, body, "Re: Root Message 1")
	assert.Contains(t, body, "Re: Re: Root Message 1")
	assert.Contains(t, body, "Re: Root Message 1 (alt)")
	assert.Contains(t, body, "Root Message 2")

	// Check for nesting indicators (highly template dependent)
	// We look for "Re: Root Message 1" inside a "thread-children" list that itself is after "Root Message 1"
	// This is brittle. A better way would be to inspect the data passed to the template if that was possible.

	// A slightly more robust check might be to look for class names if the template sets them:
	// Example: <li class="message-item thread-level-0">...Root Message 1...</li>
	//          <ul class="thread-children">
	//             <li class="message-item thread-level-1">...Re: Root Message 1...</li>

	assert.Regexp(t, regexp.MustCompile(`thread-level-0.*?Root Message 1`), body)
	assert.Regexp(t, regexp.MustCompile(`thread-level-1.*?Re: Root Message 1`), body)
	assert.Regexp(t, regexp.MustCompile(`thread-level-2.*?Re: Re: Root Message 1`), body)
	assert.Regexp(t, regexp.MustCompile(`thread-level-1.*?Re: Root Message 1 \(alt\)`), body)
	assert.Regexp(t, regexp.MustCompile(`thread-level-0.*?Root Message 2`), body)

	// Verify specific nesting for msg2 under msg1
	root1Pattern := regexp.MustCompile(`(?s)thread-level-0.*?Root Message 1.*?(<ul class="thread-children">.*?thread-level-1.*?Re: Root Message 1.*?</ul>)`)
	match1 := root1Pattern.FindStringSubmatch(body)
	if assert.NotEmpty(t, match1, "Root Message 1 and its children block not found or structured as expected") {
		childrenBlockMsg1 := match1[1] // The content of the first <ul> for msg1
		assert.Contains(t, childrenBlockMsg1, "Re: Root Message 1")
		assert.Contains(t, childrenBlockMsg1, "Re: Root Message 1 (alt)")

		// Verify specific nesting for msg3 under msg2 (which is under msg1)
		reRoot1Pattern := regexp.MustCompile(`(?s)thread-level-1.*?Re: Root Message 1.*?(<ul class="thread-children">.*?thread-level-2.*?Re: Re: Root Message 1.*?</ul>)`)
		match2 := reRoot1Pattern.FindStringSubmatch(childrenBlockMsg1) // Search within msg1's children block
		if assert.NotEmpty(t, match2, "Re: Root Message 1 and its children block not found or structured as expected") {
			childrenBlockMsg2 := match2[1]
			assert.Contains(t, childrenBlockMsg2, "Re: Re: Root Message 1")
		}
	}
